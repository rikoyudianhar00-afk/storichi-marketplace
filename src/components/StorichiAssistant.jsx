...
export default function StorichiAssistant() {
  ...
  const modeLabel = useMemo(() => mode === "seller" ? "AI Seller" : "AI Buyer", [mode]);
  ...
  async function send(raw) {
    ...
    try {
      const response = await fetch("/api/ai", {
        ...
        body: JSON.stringify({ message, mode, catalog: catalog.slice(0, 20).map(productPayload) }),
      });
      if (response.ok) {
      ...
      answer = createLocalAnswer({ message, mode, products: catalog });
    } finally {
      setBusy(false);
    }
  ...
  return <>
    ...
    {open && <div className="storichi-ai-layer" role="dialog" aria-modal="true" aria-label="Asisten Storichi">
      ...
      <section className="storichi-ai-panel">
        <header className="storichi-ai-head"><div className="storichi-ai-brand"><img src="/storichi-logo.jpg" alt="" /><div><strong>{modeLabel}</strong><small>Rekomendasi dengan persetujuan Anda</small></div></div><button type="button" className="storichi-ai-close" onClick={() => setOpen(false)} aria-label="Tutup">×</button></header>
        <div className="storichi-ai-mode" role="tablist"><button type="button" className={mode === "buyer" ? "active" : ""} onClick={() => setMode("buyer")}>Buyer</button><button type="button" className={mode === "seller" ? "active" : ""} onClick={() => setMode("seller")}>Seller</button></div>
        ...
        {mode === "seller" && !user && <p className="storichi-ai-login">Masuk terlebih dahulu untuk menggunakan AI Seller sebagai pembuat draft.</p>}
        <form className="storichi-ai-composer" onSubmit={(event) => { event.preventDefault(); send(); }}><textarea value={input} onChange={(event) => setInput(event.target.value)} maxLength={900} rows={2} placeholder={mode === "seller" ? "Contoh: buatkan draft listing akun game…" : "Cari produk atau tanyakan cara transaksi…"} /><button type="submit" disabled={busy || !input.trim()}>Kirim</button></form>
