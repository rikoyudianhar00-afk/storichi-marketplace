...
export default function RekberList() {
  ...
  async function loadGroupsAndInvitations() {
    ...
    const [{ data: memberships }, { data: pendingInvitations, error: invitationError }] = await Promise.all([
      ...
      supabase.from("rekber_invitations").select("*, purchase_request:purchase_requests(thread_id), buyer:profiles!rekber_invitations_buyer_id_fkey(id, display_name, avatar_url), seller:profiles!rekber_invitations_seller_id_fkey(id, display_name, avatar_url)").eq("third_party_id", user.id).in("status", ["buyer_approved", "accepted"]).order("created_at", { ascending: false }),
    ]);
