          ...
          {items.map((item) => {
            ...
            return (
              <article className="transaction-card" key={item.id}>
                ...
                <details className="transaction-chat-history"><summary>Riwayat chat ({messages.length} pesan)</summary><div className="transaction-chat-lines">{messages.length ? messages.map((message) => <div key={message.id} className={message.sender_id === user.id ? "is-mine" : "is-theirs"}><span>{message.visibility === "seller_whisper" ? "Whisper penjual · " : message.visibility === "buyer_whisper" ? "Whisper pembeli · " : ""}{message.content || (message.attachment_type === "image" ? "Gambar" : "Video")}</span><time>{formatDate(message.created_at)}</time></div>) : <p>Tidak ada teks chat.</p>}</div></details>
                <div className="transaction-bottom-grid"><div><small>Harga : jumlah</small><strong>Rp{Number(item.final_price || 0).toLocaleString("id-ID")} : {itemQuantity}</strong><span className="transaction-unit-price">Rp{unitPrice.toLocaleString("id-ID", { maximumFractionDigits: 2 })} / barang</span></div><div><small>{isBuyer ? "Tanggal dibeli" : "Tanggal terjual"}</small><strong>{formatDate(item.completed_at || item.seller_done_at)}</strong></div><div><small>Rating produk</small><span className="transaction-stars">{stars(item.buyer_rating)} <b>{item.buyer_rating || 0}/5</b></span><Link to={`/toko/${item.seller?.id || item.seller_id}`} className="transaction-seller-meta transaction-seller-link" aria-label={`Buka toko ${item.seller?.display_name || "Penjual"}`}><strong>{item.seller?.display_name || "Penjual"}</strong><RoleBadge profile={item.seller} size={14} /></Link></div>{item.thirdParty && <div className="transaction-third-party-meta"><small>Pihak ketiga Rekber</small><strong>{item.thirdParty.display_name}</strong><span>{item.rekber_group?.third_party_kind === "midman" ? "⚖️ Midman" : item.rekber_group?.third_party_kind === "verified" ? "Pengguna verified" : "Pengguna biasa"}</span></div>}</div>
              </article>
            );
          })}
        </div>
