-- Storichi Marketplace schema v35
-- Fix PostgREST schema-cache lookup for account neutralization.
-- Run after schema_v34.sql.

drop function if exists public.neutralize_my_rekber();
drop function if exists public.neutralize_my_rekber(text);

create or replace function public.neutralize_my_rekber(p_scope text)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if coalesce(p_scope, '') <> 'account' then
    raise exception 'Scope netralisasi tidak valid';
  end if;
  ...
  where g.status = 'active'
    and (g.buyer_id = auth.uid() or g.seller_id = auth.uid() or g.third_party_id = auth.uid() or g.midman_id = auth.uid());
  ...
  where i.status in ('pending', 'buyer_approved')
    and (i.buyer_id = auth.uid() or i.seller_id = auth.uid() or i.third_party_id = auth.uid());

  if cardinality(active_request_ids) > 0 then
    update public.purchase_requests
    set rekber_group_id = null, status = 'rejected'
  ...
  where purchase_request_id = any(active_request_ids)
     ...
     or (status in ('pending', 'buyer_approved') and (buyer_id = auth.uid() or seller_id = auth.uid() or third_party_id = auth.uid()));
  get diagnostics removed_invitations = row_count;

  return jsonb_build_object('closed_groups', closed_groups, 'reset_requests', reset_requests, 'removed_invitations', removed_invitations);
end;
$$;

grant execute on function public.neutralize_my_rekber(text) to authenticated;
